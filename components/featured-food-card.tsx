...
export default function FeaturedFoodCard({ food }: FeaturedFoodCardProps) {
  ...
  const hasAdditionalItems = food.additionalItems && food.additionalItems.length > 0
                ...
                {food.additionalItems.map((item) => (
                  <div key={item.id} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Checkbox
                        id={`featured-addon-${item.id}`}
                        checked={isSelected(item.id)}
                        onCheckedChange={(checked) => toggleAddOn(item, checked === true)}
                      />
                      <Label htmlFor={`featured-addon-${item.id}`} className="text-sm cursor-pointer">
                        {item.name}
                      </Label>
                    </div>
